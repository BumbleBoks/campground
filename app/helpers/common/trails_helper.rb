module Common::TrailsHelper
  
  def trail_link(trail)
    trail.name + trail_link_separator + trail.state.name
  end
  
  def trail_link_separator
    ","
  end
  ...
  def state_activity
    ...
    trails = Common::Trail.includes(:activities)
    categories = []
  ...
  class StateActivity < Struct.new(:state, :activity, :trails)
    ...
    def trail_collection
      ...
      trails.select do |trail|
        trail_with_activity = trail.activities.select { |trail_activity| trail_activity.id == activity.id }      
