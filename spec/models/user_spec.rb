...
describe User do
  ...
  it { should respond_to(:favorite_ofusers) }
  ...
  it { should be_invalid_with_attribute_value(:login_id, 'new') }
  it { should be_invalid_with_attribute_value(:login_id, 'create') }
  it { should be_invalid_with_attribute_value(:login_id, 'index') }
  it { should be_invalid_with_attribute_value(:login_id, 'admin') }
  it { should be_invalid_with_attribute_value(:login_id, 'superuser') }
  it { should be_invalid_with_attribute_value(:login_id, 'trailmaster') }
  ...
  describe "favorite user associations" do
    let(:other_user) { FactoryGirl.create(:user) }
    before do
      @user.save
      @user.favorite_users.create!(star_id: other_user.id)
    end
    it "should destroy favorite users associations for the user" do
      user_ids = @user.favorite_users.map { |favs| favs.id }
      @user.destroy
      user_ids.each do |id|
        Corner::FavoriteUser.find_by(id: id).should be_nil
      end
    end    
  end

  describe "favorite_of_user associations" do
    let(:other_user) { FactoryGirl.create(:user) }
    before do
      @user.save
      other_user.favorite_users.create!(star_id: @user.id)
    end
    it "should destroy favorite_of_users associations for the user" do
      user_ids = @user.favorite_ofusers.map { |favs| favs.id }
      @user.destroy
      user_ids.each do |id|
        Corner::FavoriteUser.find_by(id: id).should be_nil
      end
  ...
  describe "trade associations" do
    ...
    before do
      @user.save
      @user.trade_associations.create!(trade_id: trade.id, association_type: "interest")
    end

    it "should destroy trade associations for the user" do
      trade_assoc_ids = @user.trade_associations.map { |ta| ta.id }
      @user.destroy
